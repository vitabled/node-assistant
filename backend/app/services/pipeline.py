...
async def step_reshala(ssh: SSHSession, task: Task, req: "DeployRequest") -> None:
    """Install Reshala-Remnawave-Bedolaga support bot."""
    task.add_log(f"\n\x1b[36m{'─' * 56}\x1b[0m")
    task.add_log(f"\x1b[1;36m[Установка] Решала (Саппорт-система)\x1b[0m")
    task.add_log(f"\x1b[36m{'─' * 56}\x1b[0m")

    reshala_script = f"""\
{_APT_WAIT}
{_apt_install("git", "docker.io", "docker-compose")}

systemctl start docker 2>/dev/null || true
systemctl enable docker 2>/dev/null || true

if [ ! -d /opt/reshala ]; then
    echo "[reshala] Клонирую репозиторий Решалы..."
    git clone https://github.com/vitabled/mirror-Reshala.git /opt/reshala
else
    echo "[reshala] Репозиторий уже существует, обновляю..."
    cd /opt/reshala && git pull
fi

cd /opt/reshala
echo "[reshala] Запускаю контейнеры..."
# Ensure .env exists
if [ ! -f .env ]; then
    cat > .env << 'ENV_EOF'
# Generated by Node Assistant
BOT_TOKEN=
ADMIN_ID=
REMNNAWAVE_URL=
REMNNAWAVE_API_KEY=
...
docker compose up -d 2>&1 || docker-compose up -d 2>&1
echo "[reshala] Установка завершена."
"""
    await ssh.run_script(reshala_script, task, timeout=300)
