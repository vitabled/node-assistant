...
async def run_agent(
    ...
    # `tokens` — расход ТЕКУЩЕГО сегмента (обнуляется на границе бюджета в
    # авто-режиме), `spent` — суммарный расход за весь ответ, он не сбрасывается
    # никогда и нужен только для отчёта. См. «схему сегментов» у проверки бюджета.
    tokens = 0
    spent = 0
    segment = 1
    ...
    for step in range(limit):
        ...
        # ── схема сегментов ───────────────────────────────────────
        # `auto_token_budget` ограничивает НЕ весь ответ, а один СЕГМЕНТ —
        # отрезок работы между двумя сбросами счётчика. Раньше бюджет был
        # концом ответа: агент вставал и просил написать «Продолжи», хотя всё
        # нужное (история, вложения) уже лежало в контексте. На большой задаче
        # (импорт архива подсетей) это означало ручное «Продолжи» каждые ~1M
        # токенов.
        #
        # Теперь в АВТО-режиме граница бюджета — не стоп, а разметка: счётчик
        # сегмента обнуляется, номер сегмента растёт, цикл идёт дальше по той же
        # истории сообщений. Останов остаётся только честный:
        #   • модель вернула ответ без вызовов инструментов (задача сделана);
        #   • AUTO_FAIL_STREAK (3) безрезультатных шага подряд;
        #   • AUTO_REPEAT_STREAK (3) повтора одного вызова;
        #   • AUTO_MAX_STEPS (200) — физический предохранитель;
        #   • стоп пользователя (обрыв стрима).
        # Бесконечности нет: потолок шагов и детекторы застоя не тронуты.
        #
        # В РУЧНОМ режиме (max_steps>0) поведение прежнее: бюджет — стоп,
        # потому что там пользователь сам назначил размер ответа.
        budget = getattr(config, "auto_token_budget", 0) or AUTO_TOKEN_BUDGET
        if tokens >= budget:
            if auto:
                tokens = 0
                segment += 1
                yield {"type": "status", "phase": "thinking", "step": step + 1,
                       "steps": steps, "tokens": 0, "auto": auto,
                       "tool": (f"(бюджет токенов сегмента исчерпан: {budget}; "
                                f"продолжаю, сегмент {segment}, "
                                f"всего {spent})")}
            elif auto_tokens:
                stop_note = (
    ...
    if stop_note:
        ...
        yield {"type": "text", "delta": (
            ...
            f"один ответ (сегментов токенов: {segment}, всего {spent}). "
            f"Работа остановлена — напишите «Продолжи», файл и "
