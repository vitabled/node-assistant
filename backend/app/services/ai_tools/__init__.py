...
TOOLS: dict[str, Tool] = {t.name: t for t in (
    ...
    Tool(
        ...
        "именно меняешь. Для массового импорта данных (много записей из файла) "
        "НЕ используй этот инструмент по одной записи — используй "
        "panel_write_many пачками до 50.",
        _obj(
    ...
    Tool(
        ...
        "большой перенос не помещается в лимит шагов. При импорте большого "
        "набора данных из файла (архив/CSV/каталог) используй ЭТОТ инструмент "
        "пачками до 50 записей, НЕ panel_write по одной записи — по одной "
        "записи импорт займёт сотни шагов и упрётся в лимиты. Читай файл "
        "кусками через read_attachment, собирай пачку и пиши panel_write_many. "
        "Возвращает, что создалось "
