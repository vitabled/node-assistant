...
<!-- circle: status=done order=50 deps=[] autonomy=auto obstacle="" -->
...
<!-- circle: status=done order=60 deps=[] autonomy=auto obstacle="" -->
...
### Ф6 — done (2026-07-07)
`remnawave_client.create_host` (`POST /api/hosts`); `Template.host_template_ids` + `DeployRequest.disabled_host_template_ids`;
`pipeline.step_create_hosts` + `_map_host_optional` (после create_node, `address=FQDN`, `nodes=[uuid]`,
`inbound={configProfileUuid, configProfileInboundUuid=tpl.inbound}`, per-host warn+continue); `api/settings.py`
персистит host_template_ids; `DeployForm.tsx` чекбоксы host-шаблонов, `Templates.tsx` MultiSelect. Code-review нашёл
**HIGH баги маппинга** (тихо ломали non-default кейсы) — исправлены: remark→40, security_layer/exclude_sub_types
lowercase→UPPERCASE enum (reality dropped), visible→isDisabled (инверсия), tag≤36. Отклонения (обоснованы):
`api/settings.py`+`Templates.tsx` вместо `Settings.tsx` (реальный редактор шаблонов). Verify: `test_host_autocreate.py`
(8, incl. enum-нормализация) + весь набор **183 passed**; frontend tsc + prod-build + render-смоук clean.

### Ф5 — done (2026-07-07)
`node_ops.py`: `_DETECT_SCRIPTS` (read-only пробы per Component) + `_parse_detect` + `POST /api/node/detect`
(`NodeDetectRequest`, одна SSH-сессия, per-probe fail→unknown). `models/deploy.py`: `skip_components`.
`pipeline.py`: `_skip_component` гейтит manageable-шаги (SSH-port/optimize — безусловны); `_trafficguard_fallback`
идемпотентен (`-C || -A`). `StepProgress.tsx`: иерархическая нумерация (косметика). `DeployForm`/`DeployDashboard`:
модалка «Существующий сервер» → детект → чеклист → деплой со skip. Self-review (security): чисто; применён LOW —
`re.fullmatch` во всех domain/email-валидаторах (node_ops+models/deploy, хвостовой `\n`); defer: `-C` на whitelist-insert
(pre-existing, безвредно). Verify: `test_node_detect.py` (12) + весь набор **175 passed**; frontend tsc clean.
(Реализовано fresh general-purpose агентом после сбоя fork-делегата — fork запутался в роли, 0 правок.)
