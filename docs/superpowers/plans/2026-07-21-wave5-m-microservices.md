...
>
> **Проверено:** `pytest` 593 passed (было 564; +29 новых); `docker compose config` и
> `docker compose --profile split config/build` — оба валидны и собираются; **реальный трёхпроцессный прогон**
> (gateway + оба воркера на одном DATA_DIR) подтвердил полный жизненный цикл: монолит → перехват обязанностей
> выделенными воркерами → смерть воркеров → gateway сам возобновляет мониторинг.
>
