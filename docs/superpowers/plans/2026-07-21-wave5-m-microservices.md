...
> **Статус (2026-07-21): ОТЛОЖЕНО осознанно (не реализовано в этой сессии).** Остальные 12 планов Волны 5
> (A–L) реализованы, оттестированы и запушены. M — единственный, где распил монолита (Ф2–Ф5) сам план
> помечает **опциональным** (за compose-профилем `--profile split` с фолбэком), а его Ф1-подготовка —
> **рискованный рефактор рабочей подсистемы** (infra_billing_store на explicit account_id по ~20 функциям +
> `SharedTaskStore` поверх in-memory `task_store` + мост WS-логов). Частичный/поспешный архитектурный рефактор
> хуже, чем аккуратно отложить: план ниже — полная, готовая к реализации спецификация. Делать отдельной
> фокус-сессией с полным прогоном `pytest` после каждого извлечения сервиса.
