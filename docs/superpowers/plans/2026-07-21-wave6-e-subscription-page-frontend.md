...
# ЖИВАЯ ПАНЕЛЬ 2026-07-24 — ВСЕ ЧЕТЫРЕ БЛОКИРУЮЩИХ НЕИЗВЕСТНЫХ СНЯТЫ

> Снято на боевой панели пользователя (Remnawave 2.x) через `scripts/probe_subpage_config.py` + ручные
> запросы. Проверка PATCH велась на КЛОНЕ реальной записи, клон удалён — оригинал не тронут.

**1. Форма поля `config` — ИЗВЕСТНА.** Это структурированный объект (НЕ свободный blob) с ключами верхнего
уровня: `baseSettings, baseTranslations, brandingSettings, locales, platforms, svgLibrary, uiConfig, version`.
Внутри `platforms.{ios,android,…}.apps[].blocks[]` с локализованными `title/description/buttons` (карты
per-locale `{ru,en,fr,zh}`) и `svgIconKey`. Это формат билдера страницы подписок Remnawave.

**2. Семантика PATCH по `config` — MERGE (а не replace).** Клон с наполненным config → `PATCH {uuid, name}`
(только имя) → HTTP 200, `config` уцелел ЦЕЛИКОМ (все 8 ключей на месте). ⇒ **переименование безопасно**,
отдельная кнопка «переименовать» допустима. Наша предосторожность (не досылать `config=None`) остаётся
верной, но даже name-only PATCH оформление не трогает.

**3. КРИТИЧНО: листинг прячет `config`, деталь отдаёт.** `GET /api/subscription-page-configs` возвращает
`config: null` У ВСЕХ записей (проверено на трёх реальных). `GET /api/subscription-page-configs/{uuid}`
возвращает `config` ЦЕЛИКОМ. Значит: наш прокси-листинг честен (отдаёт что дала панель — null), но
**редактор Ф3 ОБЯЗАН тянуть деталь per-config**, а не брать config из листинга. PATCH-ответ тоже эхает
полный объект с config.

**4. Привязка config → пользователь идёт через ВНЕШНИЙ СКВАД.** Внешний сквад несёт поле
`subpageConfigUuid` (проверено: сквад «json» → `subpageConfigUuid` указывает на конфиг «service»). ⇒
контейнеру subscription-page **НЕ нужен `SUBPAGE_CONFIG_UUID` в `.env`** — выбор оформления это настройка
сквада в самой панели, наш деплой её не касается.

**Поправка к нашему же коду:** путь клона — `POST /api/subscription-page-configs/actions/clone`
`{cloneFromUuid}` (НЕ `/{uuid}/clone`). Наш `remnawave_client.clone_subscription_page_config` использует
верный `/actions/clone` — разовый пробник ошибался, бэкенд прав.

**Итог: блокеров у Плана G больше нет.** Ф6 (материализация overlay на ноде) разблокирована и bind-mount-
эксперимент выше дал рецепт; редактор поля `config` разблокирован (форма известна, PATCH — merge, тянуть
деталь per-config); `SUBPAGE_CONFIG_UUID` не требуется.
