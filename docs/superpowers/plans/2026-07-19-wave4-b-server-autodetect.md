...
> **Статус (2026-07-21):** ✅ Ф1 (backend `_DETECT_SETTINGS_SCRIPT`+`_parse_settings`, `/detect`→`{results,settings}`, коммит 502b837)
> + ✅ Ф2 (frontend: `DeployDashboard` читает `settings` → блок «Обнаружено на сервере» + preset формы). Проверено вживую на VPS.
