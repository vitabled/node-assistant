...
<!-- circle: status=done order=25 deps=[1,2] autonomy=auto obstacle="" -->
...
<!-- circle: status=done order=50 deps=[3] autonomy=auto obstacle="" -->
...
<!-- circle: status=done order=60 deps=[3,4,5] autonomy=auto obstacle="" -->
...
**стабильный `id` переживает retry** — контракт для Ф7/Ф8/Ф9), `rw/PanelDeployForm.tsx` (`validatePanelForm`
зеркалит серверные валидаторы 1:1, таргет-сегмент, reverse-proxy caddy/nginx, webhooks, extra-env редактор,
каталог Orion `GET /api/subpages` + загрузка/вставка, `toPayload` зануляет неактуальные поля), `rw/PanelWidget.tsx`
(рамка 2 подрамки панель/подписка, `useTaskStream` 8 шагов, reachability-поллинг `/api/stats/node`, onManage-хук
для Ф7). Ревью (code+security): security практически чисто (секреты не логируются/не в URL, XSS закрыт —
subpage_html только в textarea, изоляция localStorage); применены — **HIGH email-regex = серверный** (был
мягче → 422 вместо inline), M2 формат домена гейтится want* (немой дедлок), M3 pickCatalog `res.ok`+guard от
гонки, M4 bail-out в updateJobStatus (разрыв effect-loop; латентен и в эталонном DeployDashboard — не чинил
эталон), M5 стабильный id, L6 webhook whitespace = серверный, L8 reach ключ ip:port, L9 retry catch→toast,
SEC-Low file.size до readAsText. Отклонены: extra_env cleartext в localStorage (принятая архитектура как
ssh_password), L7/L10 оптимистичный статус/гранулярность отказа both (дизайн-ноты). Контракт для Ф7/Ф8/Ф9:
`PanelJobSummary{id,taskId,savedForm,createdAt,target,finalStatus}`, `onManage`-хук. Verify: tsc clean,
frontend **121 passed** (+15), render-смоук pageerrors=0.

### Ф5 — done (2026-07-08)
`subpage_store.py` (per-account HTML-стор `accounts/<id>/subpages/`, `threading.Lock` вокруг index.json
read-modify-write, атомарная запись, лимит 512KiB + `MAX_PAGES=100`, membership-guard от traversal),
`api/subpages.py` (`GET/POST/GET raw/DELETE`, 413/422/404), `rw/SubPages.tsx` (каталог + iframe `sandbox=""`
srcDoc предпросмотр + загрузка). Ревью (code+security): traversal/изоляция/sandbox/auth чисто; применены —
**SEC-Medium CSP `sandbox`+`X-Content-Type-Options: nosniff` на `/raw`** (latent-XSS при открытии в новой
вкладке, defense-in-depth к header-auth), SEC-4a лимит ≤100 страниц, code-Medium `threading.Lock` (lost-update
index.json), Low отклонение пустого html + err в превью + поправлен докстринг. Отклонены: body-parse до
лимита (nginx в контуре + серверный лимит продублирован). Verify: backend **295 passed** (+6), tsc clean.

### Ф2b — done (2026-07-08)
`api/speedtest.py` (`POST /pair` — A всегда iperf3-клиент, B=testserver → прямое подключение к постоянному
сервису, B=нода/панель → эфемерный `iperf3 -s` на свободном порту + UFW allow с IP A, cleanup в `finally`
идемпотентно + `timeout 300` self-kill backstop; `POST /xray` — тихий прогон через туннель, ссылка не течёт;
`GET /history`). `speedtest_store.history_by_kind`, `stats/SpeedTests.tsx` (выбор A/B из deploy_jobs+panel_jobs+
тест-серверов, метрики 1-3, режим пара/xray), Sidebar `stats-speedtests`, Settings дефолт-метрики. Парсеры
переиспользованы из `api/stats`. Ревью (code+security): security чисто (инъекция закрыта `ipaddress`+
`shlex.quote`, креды/ссылка не логируются, UFW точечный, изоляция per-account); применены — **SEC-Low `timeout`
вокруг эфемерного сервера**, M1 верификация бинда iperf3 (TOCTOU recheck), M3 строгий `finalStatus==='success'`
фильтр (пропускал недодеплоенные), M4 `sshPortOf` fallback на плоский `ssh_port` панели, L7 guard пустого
`kinds`, L8 нормализация IP в A==B guard, L9 disabled режим-кнопок при running, +2 теста (порт занят на B /
B недоступен → cleanup не выполняется). Отклонены/defer: M2 reconciliation утечки при обрыве B (timeout+docstring
смягчили), L5/L6 вынос install-сниппета/парсеров (рефактор вне scope, тронул бы общий stats.py), L10 xray-
источник тест-сервером (реестр не хранит креды — осознанно). Verify: backend **297 passed** (+14), tsc/npm clean.
