...
## 21. Волна 13 — суперпользователь, пользователи и роли вместо мультиаккаунтинга
> План `docs/superpowers/plans/2026-07-29-wave13-rbac-superuser-users-roles.md`. Решения Ф0 (2026-07-30):
> прежние аккаунты 2..N → пользователи одной области; `BOOTSTRAP_TOKEN` опциональный; карточки деплоя —
> вариант A (остаются пер-пользовательскими).

### 21a. Ключевое решение: «аккаунт» расщеплён на область и личность
- **Рабочая область** (`workspace_id`) — ГДЕ лежат данные. Осталась за прежним ContextVar
  `accounts.current_account`, который читают ~15 сторов, в том числе из фоновых лупов, где запроса нет вовсе.
- **Личность** — новый `users.current_user` (`DATA_DIR/users.json`): логин, роли, `token_version`, флаг
  суперпользователя, `disabled`.
- ⚠️ **Ни один стор и ни один фоновый луп не тронуты.** Это и был смысл расщепления: сохранив смысл «где
  данные» за прежней величиной, полугодовой рефакторинг превращается в одну волну.
- Миграция: аккаунт с минимальным `created_at` → суперпользователь, его каталог → область установки;
  остальные → пользователи с ролью «Наблюдатель», **пароли сохраняются**. Их прежние каталоги НЕ сливаются и
  НЕ удаляются, id остаётся в `legacy_workspace_id` (UI показывает «архив»). Автомерж потребовал бы правил
  разрешения конфликтов для 15 сторов.

### 21b. Модель привилегий (63 штуки) и разметка 271 маршрута
- `services/permissions.py`: 4 действия (`view/create/edit/execute`) × 17 доменов + 7 особых
  (`admin.users`, `admin.roles`, `admin.infrastructure`, `admin.export`, `vault.reveal`,
  `billing.purchase`, `deploy.credentials`). Удаление внутри `edit` — иначе вдвое больше галочек в UI,
  которые почти всегда стоят вместе; необратимое проходит по `execute` или особым.
- Встроенные роли: Администратор (61), Оператор (37), Финансы (24), Наблюдатель (17).
- **`RULES`** — (префикс шаблона маршрута, {метод: привилегия или кортеж}), первое совпадение, специфичное
  раньше общего. ⚠️ **Сопоставление по `request.scope["route"].path`, а не по сырому URL:** Starlette уже
  отмаршрутизировал запрос, поэтому шаблон канонический по построению — ни `%2e%2e`, ни `//`, ни `.`. Ровно та
  работа, на которой сломался денилист моста ассистента (§20h); здесь её нет, и изобретать не надо.
- ⚠️ **Запрет по умолчанию:** неразмеченный маршрут доступен ТОЛЬКО суперпользователю. Полноту держит
  `test_permissions.py::test_every_api_route_is_mapped` (обход `app.routes`, исключения — только
  `conftest.TEST_ONLY_ROUTES`). Проверено вживую: три новых роутера (`users`/`roles`/`audit`) отдали `None`,
  пока их не добавили в таблицу.

### 21c. Отзыв сессий и защита от самоблокировки
- JWT: `{sub, ver, iat}`; `ver` = `token_version`. Бампится при смене пароля, изменении ролей (и при правке
  привилегий роли — у ВСЕХ её носителей), выключении, удалении. `exp` по-прежнему нет («постоянная сессия,
  как у Google» — осознанное решение), отзыв решается версией. **До этого смена пароля не отзывала ничего.**
- Токены, выпущенные до Волны 13, версии не несут → трактуются как первая, иначе миграция разлогинила бы всех.
- Нельзя выключить, разжаловать или удалить ПОСЛЕДНЕГО суперпользователя; нельзя выключить или удалить себя.
- **Правило неусиления:** выдать роль (и создать роль) можно только обладая КАЖДОЙ её привилегией — иначе
  владелец `admin.users`/`admin.roles` собрал бы себе роль с `admin.infrastructure`. Проверяется в ОБОИХ
  местах: при создании роли (`api/roles.py::_guard_not_wider`) и при выдаче (`users.assert_can_grant`).
- Флаг суперпользователя выдаёт только суперпользователь: `admin.users` — право заводить людей, а не право
  создать себе равного.

### 21d. Что сломалось бы молча (найдено при исполнении)
- ⚠️ **Фоновые лупы обходили `accounts.list_accounts()`,** а на свежей установке Волны 13 файла
  `accounts.json` нет вовсе → мониторинг, правила, автобэкап, синк балансов и коллектор статистики **молча
  простаивали бы**. Введена `users.list_workspaces()` (УНИКАЛЬНЫЕ области живых пользователей: под одной их
  может быть много, и полить её по разу на каждого — N-кратная работа и N-кратные обращения к чужим API).
  Переведено 9 мест в 7 файлах. Регрессия — `test_users.py`.
- ⚠️ **Мост ассистента минтил токен рабочей области** (`accounts.issue_token`) → спросив у ассистента,
  пользователь получил бы то, что ему самому запрещено. Теперь `users.issue_token(user_id)`, а привилегии
  считаются в момент запроса, поэтому снятая роль обесточивает и ассистента. `ToolContext.user_id`
  обязателен, пусто = ходить нечем. То же у `mcp_server` (его фолбэк тоже минтил область).
- ⚠️ **API-токены переехали на пользователя** (`nai_<user_id>_<secret>`, поле `user_id` в записи).
  Записи БЕЗ `user_id` не резолвятся НИКОГДА — и это не недоделка: id первого прежнего аккаунта стал
  одновременно id суперпользователя и id области, поэтому старый токен резолвился бы и получил бы ПОЛНЫЕ
  права владельца вместо прав своего тенанта. Такие токены надо перевыпустить.
- ⚠️ **Сброс пароля не убивал API-токены** (они резолвятся по HMAC, а не по версии сессии). Сбрасывают пароль
  как раз тогда, когда доступ надо отобрать целиком, поэтому `reset_admin` отзывает их тоже; `--keep-tokens`
  для случая «просто забыл пароль, интеграции ломать не надо».
- ⚠️ FastAPI падает НА СТАРТЕ, если у ручки со `status_code=204` есть аннотация возврата `-> None`
  (трактует её как модель ответа). Идиома проекта — без аннотации (см. `api/vault.py`).

### 21e. Первичная настройка и сброс с хоста
- `GET /api/auth/state` → `{bootstrap}`; `POST /api/auth/bootstrap` — одноразовый, проверка «реестр пуст» и
  запись под ОДНИМ замком (иначе два запроса создали бы двух владельцев). `POST /api/auth/register`
  **удалён** (в продукте отвечает 404).
- `BOOTSTRAP_TOKEN` в окружении: пусто = выключено (конвенция `AGG_TOKEN`). `install.sh` его НЕ генерирует —
  молча включённый гейт превратил бы первый вход в «403 без объяснений».
- `node-assistant reset-admin` → `app/reset_admin.py` в одноразовом контейнере
  (`docker compose run --rm --no-deps -T backend`). Пароль ТОЛЬКО через stdin (в argv он попал бы в
  `/proc/cmdline`). `--reopen-bootstrap` намеренно НЕ сделан: снести владельца ради визарда = открыть окно, в
  котором владельцем станет любой зашедший. Пустой реестр владельца молча не создаёт.
  ⚠️ Это НЕ ослабление модели: у кого есть shell на хосте, тот уже читает `ENCRYPTION_KEY` и все Fernet-волты,
  то есть может подделать JWT любого пользователя. Команда делает законный путь удобнее незаконного.

### 21f. Тесты: как не переписывать 140 файлов
`tests/conftest.py` ставит маршрут `POST /api/auth/register` **только в тестовом процессе**, и создаёт он
пользователя с СОБСТВЕННОЙ рабочей областью и правами суперпользователя. Причина: 139 файлов заводили себе
изолированный аккаунт этой ручкой, но проверяют они СВОИ предметы (изоляцию сторов, парсеры, пайплайн), а не
модель доступа. Запись собирается низкоуровневым `users._new_user`, а не `create_user`: у создания пароля
минимум 10 символов, а тесты передают «pw», и глобальный monkeypatch на проверку лишил бы возможности
проверить её там, где она нужна. Проверки самой модели — `test_permissions.py` (создаёт пользователей с
конкретными ролями явно) и `test_users.py`.

### 21g. Журнал действий
`services/audit.py` → `DATA_DIR/audit.log` (JSONL, ротация по 5 МБ, один бэкап), `GET /api/audit`
(`admin.users`). ⚠️ Журналируется **решение гейта**, а не результат обработчика: запись делается в
`require_identity`, поэтому `allowed: true` значит «доступ разрешён», а не «операция удалась». Тела запроса в
журнале НЕТ — оно несёт пароли и SSH-креды, и журнал стал бы удобнейшим местом их сбора.

### 21h. Карточки деплоя остаются личными (вариант A)
`deploy_jobs_<id>`/`panel_jobs_<id>`/`cert_jobs_<id>` лежат в localStorage, потому что несут SSH-креды.
После расщепления они пер-пользовательские: коллега своих карточек не увидит. Это **решение, не дефект** —
общая картина серверная («Доступность серверов», ноды Remnawave). В шапке «Деплой нод» стоит строка об этом,
иначе «не вижу ноду, которую задеплоил админ» будут считать багом.

### 21i. Верификация Волны 13
- ⚠️ **Тестовый образ `ni-frontend-test` был собран до Волны 10** и не содержал `marked`/`dompurify`,
  поэтому `tsc` давал две ошибки, а `App.test.tsx` не запускался вовсе (импорт `App` → `Library` →
  `markdown.ts` → `marked`). Проверено: на `HEAD` `App.tsx` уже импортировал `Library`, а `package.json` уже
  объявлял обе зависимости — виноват образ, а не код. Пересобирать:
  `docker build --target builder -t ni-frontend-test:latest frontend`.

## 22. Ассистент: запись, адрес по провайдеру, постоянные сессии
- **Запись у ассистента была всё время** (`panel_write`, `ToolContext.readonly`), но модель об этом не
  знала: описания ручек ссылались на `panel_write`, которого в её наборе нет. Теперь `describe()` в режиме
  чтения прямо говорит «запись ВЫКЛЮЧЕНА» и что делать (снять «Только чтение» в Настройки→AI либо
  попросить готовый JSON). Иначе модель обещает записать, разбирает весь вход и упирается в отказ в конце.
- ⚠️ **`base_url` больше не хранится, а выводится из провайдера** (`AiConfig.base_url_auto` +
  `PROVIDER_BASE_URLS` в `models/settings.py`, применяется в `ai_agent.effective_target`). Закрывает класс
  ошибок «сменил провайдера — адрес остался прежним — 401 с верным ключом». Ручной режим остаётся для
  сторонних OpenAI-совместимых эндпоинтов. **Миграция:** поле появилось позже настройки, поэтому когда
  `base_url_auto` не задан ЯВНО, режим выводится из адреса (штатный/пустой → авто, иначе ручной) — иначе
  обновление молча переключило бы OpenRouter на `api.openai.com`.
- ⚠️ **Мастер-ключ шлюза действителен ТОЛЬКО против нашего контейнера.** Прежний `effective_target` брал
  его всегда, когда он сохранён, а ключ пользователя — лишь фолбэком: при выбранном шлюзе с выключенным
  контейнером наш случайный токен уезжал на публичный `base_url` → 401 «проверьте ключ». Теперь мастер-ключ
  только для `_gateway_is_ours`, внешнему адресу — ключ из формы.
- `_provider_error` называет ХОСТ, который отказал: одно и то же сообщение для «ключ неверный» и «ключ уехал
  не туда» заставляло чинить не то. Ключ в текст не попадает (тест).
- **Сессии чата постоянные, хранит их КЛИЕНТ** (`components/automation/aiSessions.ts`, localStorage
  `ai_sessions_<userId>`; сервер переписку по-прежнему не хранит). Команды `/newsession`, `/clear`,
  `/compact`. Команда распознаётся, только если это всё содержимое поля; неизвестный `/…` уходит обычным
  текстом, иначе нельзя спросить про путь `/api/...`. Лимиты 20 сессий / 200 сообщений, запись в try/catch:
  квота localStorage делится с карточками деплоя и профилями Xray.
- `POST /api/ai/compact` (`assistant.execute`) — выжимка одним тёрном БЕЗ инструментов: задача текстовая, а
  доступ к панели означал бы, что «сжатие» способно что-то изменить. ⚠️ Новая ручка `/api/ai/*` НЕ попадает
  под префикс `/api/ai/chat` — нужна своя строка в `permissions.RULES`, иначе тест покрытия падает (это и
  есть его работа).

### 22a. Вложения чата: большие файлы и картинки по своим записям
> Симптом: приложили каталог хостингов на 22,4 МБ, ассистент «нашёл 2 хостинга» и сообщил, что файл обрезан.

- ⚠️ **Обрезали его МЫ.** `MAX_TEXT_CHARS` было 40 000 и резало ВЕСЬ файл — модель видела 0,18% и делала
  единственный доступный вывод. Теперь 40 000 разделены на два разных потолка: `INLINE_TEXT_CHARS` (20 000,
  сколько уходит в промпт) и `MAX_TEXT_CHARS` (2 000 000, сколько принимаем и держим доступным). При обрезе в
  сообщение вставляется ЯВНАЯ пометка «ЭТО НЕ ВЕСЬ ФАЙЛ» с указанием, чем дочитать: молчаливый обрез — это
  когда модель уверенно отвечает по началу и объявляет отсутствующим то, что не доехало.
- **`read_attachment` / `search_attachment`** (`ai_tools`, `needs_attachment=True`) — чтение кусками по 30 000
  символов с `next_offset`/`eof` и поиск подстроки с фрагментами. Поиск ОБЫЧНЫЙ, не регулярка: выражение от
  модели на файле в сотни тысяч символов легко оказывается катастрофически backtracking-ищущим.
- ⚠️ **Картинки нельзя ни оставить, ни выбросить.** В том файле 171 картинка = 97% объёма (21 МБ из 22):
  оставить — не влезают сами данные; выбросить — теряется то, что и просят перенести (`HostingBody.media`).
  Решение: `extract_data_uris` выносит их в отдельный канал (`ToolContext.attachments[i]["images"]`,
  клиент делает то же в `AiChat.readFile`), а в тексте остаётся маркер `«изображение #N»` РОВНО НА МЕСТЕ
  картинки. `save_attachment_image(indices, prefix)` кладёт их в медиатеку и возвращает `media_id` для поля
  `media` нужной записи.
- ⚠️ **Привязка «картинка → хостинг» держится позицией маркера.** В этом файле картинки лежат не внутри
  записей, а отдельным объектом `const SHOTS={"Aeza":[…], "Hetzner":[…]}` — после выноса он схлопывается из
  21 МБ в явную читаемую карту «имя хостинга → номера картинок». Проверено на настоящем файле сквозняком:
  карта найдена, 15 картинок Aeza сохранены и привязаны к её карточке.
- Потолки: 30 картинок за вызов (каждая — запись файла на диск), 500 картинок на вложение, 25 МБ на картинку.

### 22b. Список разговоров ассистента — боковая панель
- Выпадающий `<select>` в шапке чата не держал длинные названия: заголовок задаёт пользователь (первые ~40
  символов его первого сообщения), и текст вылезал за рамку. Заменён на панель слева, как в Claude Desktop:
  `aside` шириной 224px со списком разговоров, кнопкой «Новый разговор» и удалением по наведению; в шапке —
  переключатель `PanelLeft` и ИМЯ активного разговора (название раздела и так есть в хлебных крошках
  «Автоматизация / Ассистент», дублировать его в шапке незачем).
- Обрезка держится на `truncate` + `min-w-0` у заголовка: без `min-w-0` flex-ребёнок не сжимается ниже
  содержимого, и `truncate` не срабатывает — это и была исходная причина.
- ⚠️ На экране ≤820px панель ложится ПОВЕРХ чата (`.ni-ai-sessions` в `index.css`, `position:absolute`) и
  закрыта по умолчанию: 224px из 375px не оставили бы места самому разговору.
- Тесты цепляются за `data-testid="ai-session-row"`, а не за `<option>`.
- **Проверено вживую** (vite в контейнере + бэкенд на хосте, 1280px и 375px): длинный заголовок обрезается
  многоточием, ни один элемент не выходит за свой контейнер, горизонтального скролла страницы нет,
  переключатель прячет и показывает панель, на узком экране она абсолютная с `z-index:30`.

### 22c. Строка состояния работы агента
- Симптом: инструменты в логе отработали, текста ещё нет — «вроде работает, вроде нет». Пустой лог не
  отличить от зависшего.
- Стрим получил событие **`status`** (`{phase: thinking|tools|done, step, steps, tokens}`).
  ⚠️ Первое отдаётся ДО первого обращения к провайдеру: тёрн с большим вложением идёт десятки секунд, и
  всё это время в чате не было ничего.
- Расход токенов снимается с ответа провайдера (`_usage_openai` — `total_tokens` либо сумма prompt+completion;
  `_usage_anthropic` — `input+output`) и копится по тёрнам. Провайдер не сказал → 0: показать ноль честнее,
  чем выдумать число.
- Фронт рисует строку **в конце лога**, а не в шапке: лог автопрокручивается вниз, поэтому она всегда на виду
  и стоит рядом с тем, что агент уже успел сделать. Содержимое: фаза · время · шаг N из M · токены · имя
  выполняющегося инструмента.
- Часы тикают ТОЛЬКО пока `busy`: интервал, живущий всегда, перерисовывал бы страницу раз в секунду просто так.
  Имя инструмента сбрасывается на новом «Думает» — он уже отработал, и показывать его дальше значит врать.
- `fmtTokens`/`fmtElapsed` экспортированы и проверяются напрямую: гонять стрим по секундам ради формата
  бессмысленно.

### 22d. Вложение живёт ВЕСЬ разговор, а не одно сообщение
- Симптом: приложили каталог → агент упёрся в лимит шагов → «Продолжи» → файла уже нет, агент пошёл искать
  данные в заметках (`search_notes`/`read_note`) и ответил чепухой.
- **`services/ai_attachments.py`** — вложения разговора в ПАМЯТИ процесса, ключ `(user_id, session_id)`.
  Клиент шлёт `session_id` (= id разговора из `aiSessions`) с каждым сообщением; файл прикладывается один раз.
  ⚠️ **Пустой список НЕ стирает запомненное** — иначе следующее сообщение без файла обнуляло бы работу.
- Почему в памяти, а не на диске: вложение — материал разговора, а не документ аккаунта; чат всегда идёт в
  gateway-процессе (в очередь уходят только деплои, §10d); перезапуск теряет файл, и это приемлемее вечно
  растущего каталога мусора. Границы: TTL 2 ч, 3 разговора на пользователя, 400 МБ суммарно.
- **Молчание запрещено:** пустой текст без вызовов инструментов раньше просто закрывал стрим, и в чате
  оставался пустой пузырь — не отличить от зависшего агента. Теперь это названо словами.
- **Лимит шагов — штатная ситуация на большой задаче**, поэтому сообщение говорит, что делать: «напишите
  Продолжи — файл и контекст сохранены; либо поднимите Шагов агента».
- ⚠️ Тест вытеснения: метки времени ставить БЛИЗКО к текущему моменту. Далёкое прошлое вычищается раньше как
  протухшее, и проверяется не то правило (на этом первый вариант теста и упал).
- ⚠️ **`users.current_user` — ContextVar, и установленное в тесте значение течёт в СЛЕДУЮЩИЕ тесты процесса.**
  На одиночном прогоне всё зелено, на полном падает `test_api_tokens` (список токенов фильтруется по
  личности). В тестах, которым нужна личность, ставить её через фикстуру с `set`/`reset`. И `users.bootstrap`
  в тестах не работает — реестр к тому моменту уже наполнен соседями; пользователь заводится тестовым шимом
  `POST /api/auth/register` (§21f).

### 22e. Пустой ответ = обрыв по лимиту вывода (не «модель промолчала»)
- ⚠️ **У Anthropic `max_tokens` был ЖЁСТКО зашит в 1024.** Тело одного `panel_write` для карточки хостинга
  (тарифы, заметки, оценки, media) легко больше — ответ обрывался посреди JSON, разбирать было нечего, и в
  чат приходил пустой пузырь. Теперь `AiConfig.max_tokens` (по умолчанию 8192, 256..64000) и поле «Токенов
  на ответ» в форме. OpenAI-совместимым параметр НЕ шлём: у них своё умолчание, а имя разъехалось по версиям
  API (`max_tokens` / `max_completion_tokens`).
- Тёрн теперь возвращает **`stop`** (`finish_reason` у OpenAI, `stop_reason` у Anthropic). Пустой ответ с
  `length`/`max_tokens` объясняется отдельно от «модель промолчала» — чинится он другой настройкой.
- ⚠️ **`max_steps` по умолчанию 6 → 12** (потолок 20 → 40). Шести шагов не хватало даже на разведку: сводка →
  каталог ручек → поиск во вложении → чтение куска, и предел исчерпан, а работа не начата.

### 22f. Бюджет шагов сгорал на разведке — три правки
> Живой прогон: 10 шагов, из них 6 — повторный `search_attachment` с ТЕМИ ЖЕ аргументами; ни одной записи
> не создано, финальная реплика — описание плана («Смотрю схему записи…»), который выполнить уже нечем.

- **Дедуп вызовов** (`ToolContext.calls`): повтор инструмента с идентичными аргументами возвращает
  напоминание «результат выше по переписке, не повторяй», а не данные. Шаг всё равно тратится, но модель
  разворачивается к работе. ⚠️ Подпись = имя + отсортированный JSON аргументов, поэтому
  `read_attachment(offset=0)` и `read_attachment(offset=30000)` — РАЗНЫЕ вызовы, и последовательное чтение
  файла под дедуп не попадает.
- **Стратегия в промпте — только при наличии вложения** (`describe`): не искать повторно; читать
  последовательно до `eof`; **писать после КАЖДОГО куска**, не откладывая на конец (отложенная запись не
  случается вовсе — шаги кончаются); в конце назвать offset, на котором остановился.
- **Последний шаг честно помечается.** Он идёт БЕЗ инструментов, поэтому модель на нём часто описывает план;
  выдать это за ответ — соврать. Если на последнем шаге инструменты уже использовались, к тексту дописывается
  «бюджет исчерпан, напишите Продолжи».

### 22g. Перенос каталога в ОДИН заход — измерено на живой панели
> Ассистент упирался в «10 КБ» при чтении файла из Библиотеки. Это был не лимит Библиотеки, а
> `bridge.MAX_RESULT_CHARS`: ответ моста режется, и от 22,5 МБ доезжала шапка со стилями.

- **`open_library_file(id)`** — открывает файл Библиотеки КАК ВЛОЖЕНИЕ: дальше работают
  `read_attachment`/`search_attachment`/`save_attachment_image`, включая вынос картинок. Отдельная ручка с
  offset решала бы половину задачи; так файл попадает в уже существующий конвейер. Без `id` — список файлов.
- **`panel_write_many(method, path, items[])`** — до 50 записей за вызов, с пер-записным отчётом об ошибках.
  Без этого большой перенос не помещается ни в какой лимит шагов.
- ⚠️ **КОРЕНЬ «хождения по кругу»: `_TOOL_RESULT_CAP` (12 000) был МЕНЬШЕ `ATTACHMENT_CHUNK` (30 000).**
  Инструмент отдавал 30k, а в историю попадало 12k, обрезанных ПОСРЕДИ JSON — модель получала неразбираемый
  огрызок и шла читать заново. Теперь 34 000, и связь между потолками закреплена тестом
  (`_TOOL_RESULT_CAP > ATTACHMENT_CHUNK`), а не совпадением чисел.
- **Вытеснение старых результатов** (`_trim_history`/`_trim_anthropic`, бюджет 120 000 символов): 25 кусков по
  30k — это ~200 тысяч токенов только на файл, то есть перенос упирался бы не в наши лимиты, а в окно модели.
  Вытесняется самое старое, на его место — пометка «результат вытеснен, он уже обработан».
- `max_steps` потолок 40 → **60**: измеренный перенос занял 33 вызова.
- **ИЗМЕРЕНО НА ЖИВОЙ ПАНЕЛИ** (реальный бэкенд, файл 22,5 МБ в Библиотеке): 2 открытия + поиск + 25 чтений
  по 30k + 5 пакетных записей = **33 вызова, 205 записей, 0 ошибок, история 92 666 символов, 11,7 с**.
  Воспроизводить так же: поднять бэкенд с временным `DATA_DIR`, `POST /api/library/upload`, дальше гонять
  `ai_tools.run(...)` напрямую — модель для этого не нужна, а её решения проверяются отдельно.

### 22h. Ответ переживает уход со страницы
- Симптом: переключил раздел панели — ответ потерян. Стрим жил в `AiChat` и обрывался на размонтировании
  (`return () => abortRef.current?.abort()`), а ответ по большому файлу идёт минутами.
- **`components/automation/aiRunner.ts`** — исполнитель-синглтон ВНЕ компонента. ⚠️ Он владеет НЕ ТОЛЬКО
  прогрессом, но и состоянием сессий: иначе задача решается наполовину — запрос продолжается, а писать
  ответ некуда, `setState` размонтированного компонента ничего не делает. Компонент подписывается через
  `useSyncExternalStore` (снимок = числовая версия) и при возврате просто ПОДКЛЮЧАЕТСЯ к идущему ответу.
- Автоотмены на unmount больше нет; отмена только явная — кнопка «Остановить» вместо «Отправить», пока идёт
  ответ.
- Часы считаются от `startedAt` ИСПОЛНИТЕЛЯ, а не от монтирования: вернувшись, человек должен видеть
  настоящее время работы, а не ноль.
- ⚠️ Синглтон переживает размонтирование — значит и утекает между тестами. `runner.reset()` в `beforeEach`
  рядом с `localStorage.clear()` (тот же класс граблей, что `users.current_user` на бэкенде, §22d).

### 22i. Повтор при сбое провайдера
- Симптом: после 15 вызовов — «Ошибка провайдера 500», и ВЕСЬ ответ вместе с проделанной работой потерян.
- `_post_retrying` повторяет тёрн **3 раза** (паузы 1 с и 3 с) на **5xx и сетевых обрывах**: они почти всегда
  преходящи, а обращение к провайдеру побочных эффектов не имеет.
- ⚠️ **4xx НЕ повторяем** (неверный ключ, неизвестная модель, слишком большой запрос) — само не починится, а
  повтор превратит понятную ошибку в тройную задержку перед той же ошибкой.
- Сообщение при сдаче называет вероятную причину (перегрузка провайдера / слишком большой запрос) и говорит,
  что делать: «Продолжи» — прочитанное не потеряно.
- ⚠️ В тестах подменять НЕ `mod.asyncio.sleep` (это глобальный модуль — лямбда уходит в бесконечную
  рекурсию), а константу `_RETRY_DELAYS`; она для того и вынесена.

### 22j. Сбой на длинном ответе: таймауты, безмолвное исключение, ужимание запроса
> Живой прогон: 24 вызова, СЕМЬ `panel_write` и две картинки (то есть перенос реально пошёл), затем
> «Провайдер недоступен:  » — с пустым текстом после двоеточия.

- ⚠️ **Раздельные таймауты вместо одного числа.** Было `timeout=120.0` на всё; генерация тела запроса на
  8192 токена вывода идёт минутами, и общий таймаут обрывал ЖИВОЙ ответ. Стало
  `connect=15, read=300, write=60`: подключение к недоступному хосту должно падать быстро, а чтение — ждать.
- ⚠️ **У таймаутов httpx текст ПУСТОЙ** (`str(ReadTimeout(""))` == `""`), поэтому сообщение вырождалось в
  «Провайдер недоступен: » и не говорило ничего. `_describe_exc` добавляет имя класса: `ReadTimeout` и
  `ConnectError` — разные болезни с разным лечением.
- **Каждая следующая попытка режет историю вдвое** (`shrink` в `_post_retrying` → `_trim_history`/
  `_trim_anthropic` с уменьшенным бюджетом): половина таких сбоев — «запрос слишком большой», на который
  провайдеры отвечают то 500, то таймаутом, и повтор ТЕМ ЖЕ телом спотыкается снова.

### 22k. Ответ переживает ПЕРЕЗАГРУЗКУ + параллельные инструменты
- **`services/ai_runs.py`** — цикл агента крутится ФОНОВОЙ задачей на сервере и складывает события в буфер
  разговора; HTTP-поток лишь ЧИТАТЕЛЬ буфера. Обрыв соединения (F5, потеря сети) работу не прекращает.
  Ручки: `POST /api/ai/chat` (старт + чтение), `GET /chat/state`, `GET /chat/resume`, `POST /chat/stop`.
  Ключ — `(user_id, session_id)`; повторная отправка в идущий разговор ПОДКЛЮЧАЕТСЯ к нему, а не плодит
  второго агента. Границы: TTL завершённого 15 мин, предохранитель 30 мин, 4000 событий, 50 разговоров.
  ⚠️ `asyncio.create_task` копирует контекст, поэтому фоновая задача видит `current_user`/`current_account`
  запроса — от них зависят и права моста, и выбор каталога данных.
- ⚠️ **`stop()` завершает разговор САМ, не надеясь на `finally` в задаче.** Отмена сразу после запуска
  приходит раньше, чем задача успела начать выполняться: тело не отработает, `finally` не случится, и
  разговор навсегда останется «идущим» (поймано тестом).
- Клиент: `aiRunner.resume(uid)` на монтировании спрашивает `/chat/state` и дочитывает незаконченный ответ.
  События приходят С НАЧАЛА — после F5 клиент не знает, сколько успел применить, поэтому последняя реплика
  собирается заново. Разбор потока ОДИН (`consume`) на отправку и на переподключение: разойдись они —
  восстановленный ответ отличался бы от живого.
- **Инструменты одного тёрна выполняются параллельно** (`asyncio.gather`): модель часто просит несколько
  чтений сразу, и последовательное выполнение складывало задержки. ⚠️ Изменяющие вызовы ИСКЛЮЧЕНЫ из пачки и
  идут по очереди после чтений: у них важен порядок, а два одновременных POST в один стор — гонка за файлом
  (сторы читают-меняют-пишут целиком).
