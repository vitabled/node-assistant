...
- **CSS-var conversion:** `DeployForm`, `DeployCard`, `StepProgress`, `CertsForm`, and the deploy modal (`DeployFormModal` in `DeployDashboard.tsx`) were converted from hardcoded Tailwind grays/blues to the `var(--…)` tokens / utility classes so they follow the theme. **Still hardcoded-dark (out of this phase's scope, to be themed in their own phases):** `Dashboard.tsx` (→ Ф9), the Settings sub-tab forms (Remnawave/Deploy/Optimization), `Templates.tsx`, `TrafficRules.tsx`, `components/infra/*`.
