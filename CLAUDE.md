...
- ⚠️ vitest в контейнере **недетерминированно** падает с `Failed to start worker` / `Timeout waiting for
  worker to respond` (ровно 60 с, `transform 0ms`). Это флак окружения, а не кода: тот же файл проходит при
  повторе, а заведомо рабочие файлы в тот же момент зелёные. Прежде чем чинить «падающий» тест — перезапустить.
