...
## 2a. Appearance & Theme (light / dark / system)
- **Theme MODE** replaces the old `console`/`apple-*` skins. `theme/tweaks.ts`: `ThemeMode = "light"|"dark"|"system"` (`THEME_MODES`), default **system**. `applyThemeMode(mode)` sets `data-theme` on **`document.documentElement`** (`:root`) to the resolved `light`/`dark`; `system` resolves via `matchMedia("(prefers-color-scheme: light)")` and installs a live `change` listener (module-scoped `_sysListener`, replaced on each call) so an OS light/dark switch flips the app on the fly. `resolveThemeMode` is the pure resolver. The old Apple skin CSS + `applyTheme/ThemeKey/THEMES` were removed.
- **Palettes** (`index.css`): `:root` is the default **dark** palette; `:root[data-theme="light"]` overrides the surface/text/state tokens for light (plus `.card` shadow + `.toast.*` light overrides). The accent is applied inline by `applyAccent` (theme-independent), and `--font` stays JetBrains Mono in both themes (console aesthetic); the terminal (`--term-bg`) stays dark on light.
- **Storage:** theme **mode is per-account** — `localStorage` key `ni_thememode_<accountId>` (device-global `ni_thememode` fallback on the login screen), via `loadThemeMode(accountId)`/`saveThemeMode(accountId,m)`. **Accent + density stay device-global** (`ni_accent`/`ni_density`). `App.tsx` applies the persisted mode/accent/density **once on mount**; since App is keyed by `activeId` (AuthGate), an account switch remounts it → re-reads that account's mode + re-arms the matchMedia listener.
- **Controls moved to Settings → «Тема» tab** (`Settings.tsx` `ThemeTab`, exported for tests): mode cards (Системная/Светлая/Тёмная) + accent swatches + density seg; they call `apply*`/`save*` imperatively (no state lifted to App). The old topbar «Внешний вид» (Sliders) popover + the date label were **removed**.
- **Layout reorg:** topbar now shows the **«Remnawave • онлайн» indicator** (moved from the sidebar footer) + `AccountMenu`. Sidebar `NAV_MAIN` no longer contains «Настройки» — it lives in the **sidebar footer** as a nav button.
- **CSS-var conversion:** `DeployForm`, `DeployCard`, `StepProgress`, `CertsForm`, the deploy modal (`DeployFormModal` in `DeployDashboard.tsx`), and (Ф9) `Dashboard.tsx` were converted from hardcoded Tailwind grays/blues to the `var(--…)` tokens / utility classes so they follow the theme. **Still hardcoded-dark (out of scope, to be themed in their own phases):** the Settings sub-tab forms (Remnawave/Deploy/Optimization), `Templates.tsx`, `TrafficRules.tsx`, `components/infra/*`.
- **Visual regression harness:** `frontend/tests/e2e/theme-shots.mjs` (Playwright, `@playwright/test` + chromium) — seeds a device account into `localStorage`, stubs `/api/**` with benign JSON (no backend needed), and screenshots dashboard/deploy-modal/certs/settings-theme in **both** themes. Run vite on **`127.0.0.1`** (`--host 127.0.0.1`; chromium can't reach the `localhost`→`::1` bind here) then `node tests/e2e/theme-shots.mjs http://127.0.0.1:5173`. NOTE: use `waitUntil:"commit"` — vite-dev ESM does not fire DOMContentLoaded under headless chromium in this env. Unit tests: `theme/tweaks.test.ts` (mode/accent/density) + `components/Settings.test.tsx` (`ThemeTab` wiring), with a jsdom `matchMedia` stub.
...
- **Ф9 — checker management + multi-subscription selector moved onto the Dashboard** (out of Settings): `Dashboard.tsx` now renders, above the health banner, **`CheckerControls`** (collapsible, collapsed by default — enable toggle, direct `subscription_url` field, `check_interval`/**`check_method` `<select>` (ip/status/download)**/`metrics_port`, Сохранить → `POST /api/settings/xray-checker`, Обновить → `POST /api/checker/update`, Остановить → `POST /api/checker/stop`) and **`SubscriptionSelector`** (lists the account's tracked subs from `GET /api/subscriptions/status`, polled every 15s: a `background`-toggle checkbox → `PATCH /api/subscriptions/{id}`, a config-count chip, a per-sub `last_error` line + «Обновить» → `POST /api/subscriptions/{id}/refresh`, delete → `DELETE /api/subscriptions/{id}`; an add-row posts `{url, background:true}` to `POST /api/subscriptions`, surfacing the 422 "must start with http(s)" inline). `Settings.tsx`'s old `XrayCheckerSettings` sub-block (and its render inside `DeployDefaultsTab`) was deleted — Settings → Deploy no longer has any checker UI. The empty-state banner subtitle now reads "Мониторинг не запущен — включите его в настройках мониторинга выше" (was "…настройте его в Настройки → Деплой").
...
- Subscriptions: `GET/POST /api/subscriptions`, `PATCH/DELETE /api/subscriptions/{id}`, `POST /api/subscriptions/{id}/refresh` (session-gated); `GET /internal/agg-subs` (**ungated**, internal-network only — see §4b Ф8).
