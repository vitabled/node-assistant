...
7. **DNS + SSL** (`step_ssl(…, cert_provider)`) — **branches on `cert_provider`**, per-FQDN cert (NEVER a root wildcard), installed to `/etc/ssl/certs/{domain}_fullchain.pem` + `/etc/ssl/private/{domain}.key`; issuance gated on actual `_ecc/*.cer` files (`--force` only when missing):
