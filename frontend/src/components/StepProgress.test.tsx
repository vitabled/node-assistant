...
describe("StepProgress (Ф6 restructure + Ф2-wave1 test-tools step)", () => {
  it("has 14 deploy steps with the renamed/split labels", () => {
    expect(DEPLOY_STEPS).toHaveLength(14);
    // Ф2 wave1: test-tools step inside «Оптимизация ОС»
    expect(DEPLOY_STEPS[4]).toBe("Тест-инструменты");
  ...
  it("renders the three collapsible groups", () => {
    ...
    render(<StepProgress currentStep={12} totalSteps={14} status="running" />);
  ...
  it("orders «Маскировочный сайт» before «WARP Native» (masking runs first)", () => {
    const { container } = render(<StepProgress currentStep={12} totalSteps={14} status="running" />);
  ...
  it("keeps standalone steps outside any group (Подключение, SSL)", () => {
    render(<StepProgress currentStep={10} totalSteps={14} status="running" />);
  ...
  it("renders every step 1..14 exactly once (no gap/overlap between groups)", () => {
    // pending + no active step → groups default-open, so all 14 rows render.
    // Numbering is hierarchical (Ф5: 1, 2, 3.1…6.4) — the invariant is that
    // every label renders exactly once and the badges cover all 14 steps.
    const { container } = render(<StepProgress currentStep={0} totalSteps={14} status="pending" />);
    const text = container.textContent || "";
    for (const label of DEPLOY_STEPS) {
      const count = text.split(label).length - 1;
      expect(count, `label «${label}»`).toBe(1);
    }
    const badges = ["1.", "2.", "3.1.", "3.2.", "3.3.", "4.1.", "4.2.", "4.3.", "4.4.", "5.", "6.1.", "6.2.", "6.3.", "6.4."];
    for (const badge of badges) {
      expect(text, `badge «${badge}»`).toContain(badge);
    }
    // no flat 7..14 badges remain after the hierarchical renumbering
    expect(text).not.toMatch(/(?<![\d.])(?:1[0-4]|[7-9])\./);
  });

  it("shows 100% and marks groups done on success", () => {
    const { container } = render(<StepProgress currentStep={14} totalSteps={14} status="success" />);
