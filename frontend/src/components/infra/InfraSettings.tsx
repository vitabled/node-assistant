...
export function InfraSettings() {
  ...
  if (!s) return <Page><div className="py-16 text-center text-[var(--t-faint)]"><Loader2 size={18} className="animate-spin inline" /></div></Page>;
  ...
  return (
    <Page>
      <PageHeader icon={<SlidersHorizontal size={16} className="text-[var(--accent)]" />} title="Настройки биллинга"
      ...
      <div className="max-w-lg flex flex-col gap-5">
        ...
        <div>
          <label className="text-[11px] font-medium text-[var(--t-low)] uppercase tracking-widest">Курсы (1 единица = X RUB)</label>
          <div className="grid grid-cols-3 gap-3 mt-1">
            {CURRENCIES.map(c => (
              <div key={c} className="flex flex-col gap-1">
                <span className="text-[11px] text-[var(--t-faint)]">{c}</span>
                <input type="number" value={String(s.fxRates[c] ?? "")} onChange={e => setRate(c, e.target.value)} className={inputCls} />
              </div>
            ))}
          </div>
          <p className="text-[11px] text-[var(--t-faint)] mt-1">Курсы вводятся вручную (авто-обновление FX не подключено).</p>
        </div>
        ...
        <button onClick={save} disabled={saving}
          className="self-start flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium bg-[var(--accent)] hover:bg-[var(--accent-hi)] text-[var(--primary-ink)] disabled:opacity-50">
