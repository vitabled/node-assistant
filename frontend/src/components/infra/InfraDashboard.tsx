...
export function InfraDashboard() {
  ...
  return (
    <Page>
      <PageHeader icon={<PieChart size={16} className="text-blue-400" />} title="Dashboard биллинга"
        subtitle="Сводные финансовые метрики инфраструктуры"
        actions={<button onClick={load} className="p-2 rounded-md bg-gray-800 hover:bg-gray-700 text-gray-300"><RefreshCw size={13} /></button>} />
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-5 flex items-center gap-4">
              <Wallet size={22} className="text-blue-400" />
              <div>
                <p className="text-[11px] uppercase tracking-widest text-gray-500">Общий баланс</p>
                <p className="text-2xl font-semibold text-white tabular-nums">{fmtNum(d.totalBalance, d.baseCurrency)}</p>
                <p className="text-[11px] text-gray-600">по всем провайдерам, в базовой валюте</p>
              </div>
            </div>
            <div className={`rounded-xl border p-5 flex items-center gap-4 ${b?.critical ? "border-red-800/50 bg-red-950/40" : "border-gray-800 bg-gray-900/40"}`}>
              <TrendingDown size={22} className={b?.critical ? "text-red-400" : "text-blue-400"} />
              <div className="flex-1">
                <p className="text-[11px] uppercase tracking-widest text-gray-500">Burn Rate</p>
                <p className={`text-2xl font-semibold ${b?.critical ? "text-red-300" : "text-white"}`}>
                  {b?.daysLeft != null ? `≈ ${b.daysLeft} дн до нуля` : "нет расходов"}
                </p>
                <p className="text-[11px] text-gray-600">
                  {fmtNum(b?.hourly ?? 0)}/ч · {fmtNum(b?.daily ?? 0)}/д · {fmtNum(b?.monthly ?? 0)}/мес
                </p>
                {b?.critical && <p className="text-[11px] text-red-400 flex items-center gap-1 mt-0.5"><AlertTriangle size={11} /> Осталось менее 7 дней!</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4">
              <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-3">Распределение расходов по провайдерам</p>
...
export function Donut({ data }: { data: { provider: string; total: number }[] }) {
  ...
  if (total <= 0) return <div className="h-44 flex items-center justify-center text-gray-700 text-xs">Нет данных о расходах.</div>;
  ...
  return (
    <div className="flex items-center gap-5">
      <svg viewBox="0 0 160 160" className="w-40 h-40 shrink-0">
        ...
        <text x="80" y="84" textAnchor="middle" fill="#e5e7eb" fontSize="12" fontWeight="600">{Math.round(total).toLocaleString("ru-RU")}</text>
      </svg>
      <div className="flex flex-col gap-1.5 text-xs min-w-0">
        {data.map((x, i) => (
          <div key={i} className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-gray-300 truncate">{x.provider}</span>
            <span className="text-gray-500 ml-auto tabular-nums">{Math.round((x.total / total) * 100)}%</span>
          </div>
        ))}
      </div>
    </div>
...
export function MonthLine({ data }: { data: { month: string; total: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-44">
      <line x1={P} x2={W - P} y1={y(0)} y2={y(0)} stroke="#1f2937" />
      ...
      {data.map((d, i) => <text key={i} x={x(i)} y={H - 8} textAnchor="middle" fill="#6b7280" fontSize="9">{d.month.slice(5)}</text>)}
    </svg>
  );
}
